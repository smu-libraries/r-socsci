...
Session 4 asked *"which test?"* from your variable types. Regression asks the same kind of question — now the **outcome's type** picks your model:
...
Whatever the model, the core skill is the same: **read one coefficient at a time, in the units of your outcome.** If a predictor is categorical, its coefficient is the difference from a **reference** group.

::: {.callout-tip appearance="simple"}
You already know the shape of this from school: $y = mx + c$. Regression just adds *uncertainty* (are the slope and intercept real?) and *more predictors*.
:::
...
::: {.callout-tip appearance="simple"}
This is the $y = mx + c$ line from secondary school, dressed up: $\beta_0$ is the intercept ($c$), $\beta_1$ is the slope ($m$). Regression's job is to find the best-fitting $m$ and $c$ from your data — and tell you whether they're statistically real.
:::
...
::: {.callout-tip appearance="simple" title="Saving a gtsummary / gt table"}
These tables render in RStudio's **Viewer** pane (lower-right). Select the table, copy (Ctrl/Cmd + C), and paste straight into Word or Google Docs.
:::
...
::: incremental
- **Choosing** the right test for your variables — and being able to say *why* (remember the map!).
- **Checking** whether the result can be trusted — assumptions, sample size, plain common sense.
- **Interpreting** the output honestly — a significant p-value isn't proof, and a non-significant one isn't "no effect".
...
::: incremental
- Let AI handle the **syntax** — that's the cheap, replaceable part now.
- *You* own the **judgement**: the choices, the checks, the honest interpretation and reporting.
- That judgement is exactly what we've been building this whole workshop — and it's what will still matter long after today's tools have been replaced by something newer.
:::

::: {.callout-tip appearance="simple"}
Treat AI as a **capable assistant you supervise**, not an oracle you obey. Always verify what it hands you against what you now know how to check yourself. That habit is what separates an analyst from someone who just runs code.
:::
...
- More table packages (apaTables, gtsummary summary tables)
